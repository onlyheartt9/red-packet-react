...
const StateComponents = ({ status } : {status: statusType} ) => {
  ...
  const ballColor = ConfigClass.Ball_Color[status];
  const text = ConfigClass.Text[status];
...
const ContextComponent = ({ status, message = {} } : { status: statusType, message: packetType}) => {
  return (<>
  ...
  { status !== "Not_Participated" && (<ModalContent>
    {(onClose) => (
      <>
        ...
        <ModalBody>
          <p>
            开始时间: {message?.startTime ? 
            new Date(message?.startTime).toLocaleString() :
            'Not Data'}
          </p>
          <p>
             红包金额: {message?.amount ?? 0 }
          </p>
          <p>
            代币类型：{message?.collectType ?? ''}
          </p>
          <p>
            发起人： {message?.creator ?? 'none'}
          </p>
        </ModalBody>
        <ModalFooter>
        </ModalFooter>
